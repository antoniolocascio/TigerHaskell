...
{
    long i;
    long *a = GC_MALLOC((size+1) * sizeof(long));
	a[0] = size;
    for (i = 1; i <= size; i++)
		a[i] = init;
    return a+1;
...
string *substring(string *s, long first, long n)
{
    ...
    {
		string *t = GC_MALLOC(sizeof(string) + n);
		long i;
		t->length = n;
		for (i = 0; i < n; i++)
...
int main()
{
	extern long _tigermain_0_0(long);
    int i, lelem=sizeof(string)+1;;
	string *p=GC_MALLOC(256*lelem);
    for (i = 0; i < 256; i++) {
		consts[i]=p+i*lelem;
		consts[i]->length = 1;
		consts[i]->chars[0] = i;
    }
    return _tigermain_0_0(0 /* static link!? */ );
}
